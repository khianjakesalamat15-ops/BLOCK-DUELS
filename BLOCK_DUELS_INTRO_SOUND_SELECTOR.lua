...
-- ============================================================
-- INTRO SYSTEM (replaced with uploaded AdaptDuels INTRO)
-- ============================================================
local INTRO_MUSIC_OPTIONS = {
    {name="OFF",    url=nil, file=nil},
...
local function getIntroAssetLoader()
...
local function cacheIntroSong(option)
    if not option or not option.url or not option.file then return nil end
    local loader = getIntroAssetLoader()
    if not loader or type(writefile) ~= "function" then return nil end

    local okExisting, exists = pcall(function()
        return type(isfile) == "function" and isfile(option.file)
    end)
    if okExisting and exists then
        local okAsset, asset = pcall(loader, option.file)
        if okAsset and asset and tostring(asset) ~= "" then return asset end
    end

    local okData, data = pcall(function() return game:HttpGet(option.url) end)
    if not okData or not data or data == "" then return nil end
    if not pcall(function() writefile(option.file, data) end) then return nil end
...
local function playIntroMusic()
    ...
    if not option or not option.url then return end

    task.spawn(function()
        local soundId = cacheIntroSong(option)
        if token ~= introPlaybackToken or not soundId then return end
        local SoundService = game:GetService("SoundService")
        local sound = Instance.new("Sound")
        sound.Name = "AdaptDuelsIntroMusic"
...
local function loadIntroSoundConfig()
    local saved = _configState and tonumber(_configState["introSoundIndex"])
    if saved and saved >= 1 and saved <= #INTRO_MUSIC_OPTIONS then
        selectedIntroMusic = saved
    end
end
...
_G.__AdaptDuelsRunIntro = function()
    local introGuiParent = PGui
    local origSize = Container.Size
    local wasMinimizedBeforeIntro = (Container.Visible == false)
    ...
    task.spawn(function()
        ...
        local function finishIntro()
            ...
            introTween(tapAnywhere,0.25,{
                TextTransparency=1,
                TextStrokeTransparency=1
            })
            introTween(discordInvite,0.25,{
                TextTransparency=1,
                TextStrokeTransparency=1
            })
            ...
            task.delay(0.35,function()
                ...
                Container.Size = origSize
                Container.Visible = not wasMinimizedBeforeIntro
                ShowBtn.Visible = wasMinimizedBeforeIntro
            end)
        end
...
;(function()
    ...
    do
        ...
        -- ============================================================
        do
            loadIntroSoundConfig()

            local introSoundRow = Instance.new("Frame")
            introSoundRow.Name = "IntroSoundRow"
            introSoundRow.Size = UDim2.new(1, 0, 0, 42)
            introSoundRow.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
            introSoundRow.BackgroundTransparency = ROW_ALPHA
            introSoundRow.BorderSizePixel = 0
            introSoundRow.LayoutOrder = 93.7
            introSoundRow.Parent = p
            addCorner(introSoundRow, 10)
            addStroke(introSoundRow, Color3.fromRGB(55, 55, 55), 1)

            local introSoundLabel = Instance.new("TextLabel", introSoundRow)
            introSoundLabel.Name = "Label"
            introSoundLabel.BackgroundTransparency = 1
            introSoundLabel.Text = "INTRO SOUND"
            introSoundLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
            introSoundLabel.TextSize = 12
            introSoundLabel.Font = Enum.Font.GothamBold
            introSoundLabel.TextXAlignment = Enum.TextXAlignment.Left
            introSoundLabel.Position = UDim2.new(0, 12, 0, 8)
            introSoundLabel.Size = UDim2.new(0.5, 0, 0, 16)
            introSoundLabel.ZIndex = 5

            local introSoundUl = Instance.new("Frame", introSoundRow)
            introSoundUl.Size = UDim2.new(0, 86, 0, 1)
            introSoundUl.Position = UDim2.new(0, 12, 0, 25)
            introSoundUl.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
            introSoundUl.BackgroundTransparency = 0.5
            introSoundUl.BorderSizePixel = 0
            addCorner(introSoundUl, 1)

            local introSoundCont = Instance.new("Frame", introSoundRow)
            introSoundCont.AnchorPoint = Vector2.new(1, 0.5)
            introSoundCont.Position = UDim2.new(1, -10, 0.5, 0)
            introSoundCont.Size = UDim2.new(0, 170, 0, 26)
            introSoundCont.BackgroundColor3 = Color3.fromRGB(18, 18, 18)
            introSoundCont.BorderSizePixel = 0
            introSoundCont.ZIndex = 5
            addCorner(introSoundCont, 5)
            addStroke(introSoundCont, Color3.fromRGB(55, 55, 55), 1)

            local introArW = 28

            local introPrev = Instance.new("TextButton", introSoundCont)
            introPrev.Size = UDim2.new(0, introArW, 1, 0)
            introPrev.Position = UDim2.new(0, 0, 0, 0)
            introPrev.BackgroundTransparency = 1
            introPrev.BorderSizePixel = 0
            introPrev.Text = "<"
            introPrev.TextColor3 = Color3.fromRGB(200, 200, 200)
            introPrev.Font = Enum.Font.GothamBold
            introPrev.TextSize = 14
            introPrev.ZIndex = 6

            local introSoundValueLabel = Instance.new("TextLabel", introSoundCont)
            introSoundValueLabel.Name = "IntroSoundValue"
            introSoundValueLabel.Position = UDim2.new(0, introArW, 0, 0)
            introSoundValueLabel.Size = UDim2.new(1, -introArW * 2, 1, 0)
            introSoundValueLabel.BackgroundTransparency = 1
            introSoundValueLabel.Text = INTRO_MUSIC_OPTIONS[selectedIntroMusic].name
            introSoundValueLabel.TextColor3 = Color3.fromRGB(220, 220, 220)
            introSoundValueLabel.Font = Enum.Font.GothamBold
            introSoundValueLabel.TextSize = 10
            introSoundValueLabel.ZIndex = 6
            introSoundValueLabel.TextScaled = true

            local introNext = Instance.new("TextButton", introSoundCont)
            introNext.AnchorPoint = Vector2.new(1, 0)
            introNext.Position = UDim2.new(1, 0, 0, 0)
            introNext.Size = UDim2.new(0, introArW, 1, 0)
            introNext.BackgroundTransparency = 1
            introNext.BorderSizePixel = 0
            introNext.Text = ">"
            introNext.TextColor3 = Color3.fromRGB(200, 200, 200)
            introNext.Font = Enum.Font.GothamBold
            introNext.TextSize = 14
            introNext.ZIndex = 6

            local function setIntroSoundIndex(nextIndex)
                if nextIndex < 1 then nextIndex = #INTRO_MUSIC_OPTIONS end
                if nextIndex > #INTRO_MUSIC_OPTIONS then nextIndex = 1 end
                selectedIntroMusic = nextIndex
                introSoundValueLabel.Text = INTRO_MUSIC_OPTIONS[selectedIntroMusic].name
                _configState["introSoundIndex"] = selectedIntroMusic
                _autoSaveConfig()
            end

            introPrev.MouseButton1Click:Connect(function()
                setIntroSoundIndex(selectedIntroMusic - 1)
            end)

            introNext.MouseButton1Click:Connect(function()
                setIntroSoundIndex(selectedIntroMusic + 1)
            end)

            hoverRow(introSoundRow, ROW_ALPHA)
            _AK._setIntroSoundUI = function()
                introSoundValueLabel.Text = INTRO_MUSIC_OPTIONS[selectedIntroMusic].name
            end
        end
    end
...
-- Run the replacement INTRO after the GUI/config have been initialized.
task.defer(function()
    pcall(function()
        loadIntroSoundConfig()
